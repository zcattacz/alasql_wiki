...
#### # Create or switch to a new release branch `git flow release start x.y.z` (in source tree click "git flow" at the top right). Name it exactly as the new version number (for example "0.2.0"). 
     ...
     { git branch | grep -m 1 -q 'release/' && eval "git checkout `git branch | grep -m 1 -o 'release/.*'`" } || run "Create and switch to a new release branch" "git stash --quiet; git flow release start $thisVersion ; git stash pop --quiet"
